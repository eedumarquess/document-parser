...
import { RetentionPolicyService } from '@document-parser/shared-kernel';

export const WORKER_QUEUE_PUBLICATION_OWNER_SERVICE = 'document-processing-worker';
...
export type WorkerQueuePublicationFinalizationMetadata = {
...
export function buildWorkerQueuePublicationOutboxRecord(input: {
  outboxId: string;
  ...
  finalizationMetadata: WorkerQueuePublicationFinalizationMetadata;
  now: Date;
}): QueuePublicationOutboxRecord {
  return {
    outboxId: input.outboxId,
    ownerService: WORKER_QUEUE_PUBLICATION_OWNER_SERVICE,
...
export class QueuePublicationOutboxDispatcherService implements OnModuleInit, OnModuleDestroy {
  private readonly leaseOwner = `worker-outbox-dispatcher:${process.pid}:${randomUUID()}`;
  ...
  private async dispatchRecord(record: QueuePublicationOutboxRecord): Promise<void> {
    ...
    try {
      ...
      await this.metrics.increment({
        name: 'worker.queue_publication_outbox.published',
      ...
      await this.metrics.increment({
        name: 'worker.queue_publication_outbox.publish_failed',
        tags: this.buildMetricTags(record)
      });
      await this.logging.log({
        level: 'error',
        message: 'Worker outbox publication failed',
  ...
  private async finalizePublished(
    ...
    const metadata = record.finalizationMetadata as WorkerQueuePublicationFinalizationMetadata | undefined;
